...
if (isset($_POST["submit"])) {
  ...
  if (count($errors)>0) {
    ...
    $sql = "INSERT INTO users (firstName, middleName, lastName, userType, Email, Password) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($conn);
    $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
    if ($prepareStmt) {
      mysqli_stmt_bind_param($stmt, "ssssss", $firstName, $middleName, $lastName, $userType, $email, $password);
